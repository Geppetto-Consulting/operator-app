...
def user_activation_email(current_site, user_id):
    try:
        ...
        subject = f"{user.first_name or user.display_name or user.email} has been activated on Promptable Operator"

        context = {"email": str(user.email), "profile_url": current_site + "/profile"}

        # Send email to user
        html_content = render_email_template("emails/user/user_activation.html", context)
