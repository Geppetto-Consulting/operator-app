...
def user_deactivation_email(current_site, user_id):
    try:
        ...
        subject = f"{user.first_name or user.display_name or user.email} has been deactivated on Promptable Operator"

        context = {"email": str(user.email), "login_url": current_site + "/login"}

        # Send email to user
        html_content = render_email_template("emails/user/user_deactivation.html", context)
