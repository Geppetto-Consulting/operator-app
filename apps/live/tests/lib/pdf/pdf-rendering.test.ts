...
import { renderDocToPdfBuffer } from "@/lib/pdf";
...
describe("PDF Rendering Integration", () => {
  describe("renderDocToPdfBuffer", () => {
    it("should render empty document to valid PDF", async () => {
      ...
      const buffer = await renderDocToPdfBuffer(doc);
  ...
  describe("page options", () => {
    it("should support different page sizes and verify content renders", async () => {
      ...
      const a4Buffer = await renderDocToPdfBuffer(doc, { pageSize: "A4" });
      const letterBuffer = await renderDocToPdfBuffer(doc, { pageSize: "LETTER" });
    ...
    it("should support landscape orientation and verify content", async () => {
      ...
      const portraitBuffer = await renderDocToPdfBuffer(doc, { pageOrientation: "portrait" });
      const landscapeBuffer = await renderDocToPdfBuffer(doc, { pageOrientation: "landscape" });
  ...
  describe("metadata rendering", () => {
    it("should render user mentions with resolved display name", async () => {
      ...
      const buffer = await renderDocToPdfBuffer(doc, { metadata });
      const text = await extractPdfText(buffer);
  ...
  describe("noAssets option", () => {
    it("should render text but skip images when noAssets is true", async () => {
      ...
      const buffer = await renderDocToPdfBuffer(doc, { noAssets: true });
      const text = await extractPdfText(buffer);
